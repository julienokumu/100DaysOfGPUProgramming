...
__global__ void sgemm_1Dblocktiling(const half* A, const half* B, half* C, int M, int N, int K, float alpha, float beta) {
	...
	for (int t = 0; t < (K + BLOCK_SIZE - 1) / BLOCK_SIZE; t++) {
		...
		for (int i = 0; i < TILE_WIDTH; i++) {
			int col_idx = blockIdx.x * BLOCK_SIZE * TILE_WIDTH + tx + i * BLOCK_SIZE;
			...
			for (int k = 0; k < BLOCK_SIZE; k++) {
				tmp[i] += __half2float(As[ty][k]) * __half2float(Bs[k][ty]);
			}
			__syncthreads();
		}
	}
...
extern "C" void solve(const half* A, const half* B, half* C, int M, int N, int K, float alpha, float beta) {
	...
	sgemm_1Dblocktiling<<<blocksPerGrid, threadsPerBlock>>>(A, B, C, M, N, K, alpha, beta);
}
